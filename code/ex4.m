...
clc,clear
[t,x] = ode45(@species,[0,1500],[2,2,2]);
temp = 1;
temp1 = 1;
x(:,1) = x(:,1)-2;
x(:,2) = x(:,2)-2;
...
Hn1 = 4;Hn2 = 4;Hn3 = 4;%Intrinsic growth rate
...
%% 1 and 2
% H1 = Hn1*(alpha1*(1-k1*(PH-PH1)^2*(T-T1)^2)+(1-alpha1)*(1-k1*(PH-PH4)^2*(T-T4)^2))*(1-k2*(M-M1)^2)*x(1)*(1-x(1)/xm1);
% H2 = Hn2*(beta1*(1-k1*(PH-PH2)^2*(T-T2)^2)+(1-beta1)*(1-k1*(PH-PH3)^2*(T-T3)^2))*(1-k2*(M-M2)^2)*x(2)*(1-x(2)/xm2);
% X1 = (7*sqrt(H1)+5)/122;
% X2 = (7*sqrt(H2)+5)/122;
% o = [X1;X2];
